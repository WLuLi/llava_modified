...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.vision_gate_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.vision_up_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.vision_down_proj = nn.Linear(self.intermediate_size, self.hidden_size, bias=False)
    ...
    def forward(self, x, img_masks=None):
        ...
        processed_language_hidden_states = self.mlp_process(language_hidden_states, self.gate_proj, self.up_proj, self.down_proj)

        # Combine processed features
        combined_features = processed_vision_hidden_states + processed_language_hidden_states
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.vision_q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        ...
        with torch.no_grad():
            self.vision_q_proj.weight.copy_(self.q_proj.weight)
            self.vision_k_proj.weight.copy_(self.k_proj.weight)
            self.vision_v_proj.weight.copy_(self.v_proj.weight)
    ...
    def forward(
        ...
        if self.pretraining_tp > 1:
            ...
            query_states = [F.linear(language_hidden_states, query_slices[i]) for i in range(self.pretraining_tp)]
            query_states = torch.cat(query_states, dim=-1)

            key_states = [F.linear(language_hidden_states, key_slices[i]) for i in range(self.pretraining_tp)]
            key_states = torch.cat(key_states, dim=-1)

            value_states = [F.linear(language_hidden_states, value_slices[i]) for i in range(self.pretraining_tp)]
            value_states = torch.cat(value_states, dim=-1)

        else:
            query_states = self.q_proj(language_hidden_states)
            key_states = self.k_proj(language_hidden_states)
            value_states = self.v_proj(language_hidden_states)

        if self.pretraining_tp > 1:
            key_value_slicing = (self.num_key_value_heads * self.head_dim) // self.pretraining_tp
            query_slices = self.vision_q_proj.weight.split((self.num_heads * self.head_dim) // self.pretraining_tp, dim=0)
            key_slices = self.vision_k_proj.weight.split(key_value_slicing, dim=0)
            value_slices = self.vision_v_proj.weight.split(key_value_slicing, dim=0)

            vision_query_states = [F.linear(vision_hidden_states, query_slices[i]) for i in range(self.pretraining_tp)]
            vision_query_states = torch.cat(vision_query_states, dim=-1)

            vision_key_states = [F.linear(vision_hidden_states, key_slices[i]) for i in range(self.pretraining_tp)]
            vision_key_states = torch.cat(vision_key_states, dim=-1)

            vision_value_states = [F.linear(vision_hidden_states, value_slices[i]) for i in range(self.pretraining_tp)]
            vision_value_states = torch.cat(vision_value_states, dim=-1)
        
        else:
            vision_query_states = self.vision_q_proj(vision_hidden_states)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, img_masks)
        hidden_states = residual + hidden_states
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        img_masks: Optional[torch.LongTensor] = None,
